...
POSSIBILITY_PURE_NUM = 1.0  # 需要产生的纯数字 -->0.2
POSSIBILITY_PURE_ENG = 0  # 需要产生的英语 -->0.1
POSSIBILITY_DATE = 0  # 需要产生的纯日期 -->0.1
...
def random_font(font_path):
    ...
    while rand_font == '.DS_Store':
        rand_font = random.choice(font_list)
    return font_path + rand_font


# 产生一张图的所有的文字
def generate_all(bground_list, image_name, label_name, charset, font_dir):
    # 先创建一张图，宽度和高度都是随机的
    ...
    while True:
        ...
        one_row_labels = generate_row(i, y, background_image=image, image_width=w, charset=charset, font_dir=font_dir)
...
def generate_row(i, y, background_image, image_width, charset, font_dir):
    # logger.debug("---------------------------------")
    ...
    while True:
        ...
        next_x, label = process_one_sentence(next_x, y, background_image, image_width, charset, font_dir)
...
def create_one_sentence_image(charset, font_dir, corpus=None):
    ...
    randome_intefer_line(words_image, POSSIBILITY_WORD_INTEFER, INTERFER_WORD_LINE_NUM,
                         INTERFER_WORD_LINE_WIGHT)  # 给单个文字做做干扰
...
def test_font(font_name):
    ...
    font_size = 20
    font = ImageFont.truetype(font_name, font_size)
    width, height = caculate_text_shape(word, font)
    words_image = Image.new('RGBA', (width, height))
    draw = ImageDraw.Draw(words_image)
    draw.text((0, 0), word, fill='black', font=font)
    words_image.save('../crnn/data/data_generator/font1.png')
...
if __name__ == '__main__':
    ...
    for num in range(0, total):
        ...
        generate_all(all_bg_images, image_name, label_name, charset, font_dir)
        logger.info("已产生[%s]", image_name)

    # test_font('../crnn/data/data_generator/font_old/华文细黑.ttf')
