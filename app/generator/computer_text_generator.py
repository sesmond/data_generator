...
class ComputerTextGenerator(object):
    ...
    @classmethod
    def __generate_horizontal_text(cls, bg_path, texts, font, text_color, font_size, space_width):
        """
        ...
        for text in texts:
            ...
            if len(labelLists) == 0:
                ...
                for i, w in enumerate(words):
                    txt_draw.text((x0 + sum(words_width[0:i]) + i * int(space_width), y0), w, fill=fill,
                                  font=image_font)
    ...
    @classmethod
    def __generate_vertical_text(cls, bg_path, texts, font, text_color, font_size, space_width):
        image_font = ImageFont.truetype(font=font, size=font_size)
        txt_img = image_utils.read(bg_path)
        txt_draw = ImageDraw.Draw(txt_img)
        img_w, img_h = txt_img.size
