...
def load_dict(lang):
    ...
    lang_dict = []
    with open(os.path.join('config/dicts', lang + '.txt'), 'r', encoding="utf8", errors='ignore') as d:
...
def load_fonts(lang):
    ...
    if lang == 'cn':
        return [os.path.join('config/fonts/cn', font) for font in os.listdir('config/fonts/cn')]
    else:
        return [os.path.join('config/fonts/latin', font) for font in os.listdir('config/fonts/latin')]
